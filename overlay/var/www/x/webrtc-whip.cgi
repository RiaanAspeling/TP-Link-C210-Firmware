#!/bin/sh
#
# Local override of raptor's WHIP signalling proxy. Two changes vs upstream:
#
#  1. Require the WebUI session. Upstream has no auth at all, yet it supplies
#     the [webrtc] credentials to rwd on the caller's behalf — so anyone who
#     could reach this endpoint could open a video session anonymously.
#
#  2. Pass the SDP answer through byte-exactly. Upstream did
#         sdp="$(cat "$body_file")" ... printf '%s' "$sdp"
#     and command substitution strips trailing newlines, so rwd's final CRLF
#     lost its LF. Chrome then reads the dangling CR as line content and
#     rejects the answer with "Invalid SDP line" on the last a=candidate,
#     making the preview fail with no usable error. Streaming the file
#     straight out preserves the terminator.

# Authenticate BEFORE `set -eu`. auth.sh dereferences CGI variables that are
# absent on non-browser requests (HTTP_ACCEPT, HTTP_COOKIE); under `set -u` the
# shell aborts before it can emit the 401, uhttpd sees an empty response and
# turns it into a confusing 502.
. /var/www/x/auth.sh
require_auth
